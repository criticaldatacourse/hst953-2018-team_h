WITH pvt AS (
  ...
  , CASE
        when itemid = 50810 then 'Hct'
        when itemid = 51221 then 'Hct'
      ELSE null
      END AS label
  , -- add in some sanity checks on the values
    ...
    CASE
      when le.itemid = 50810 and le.valuenum >    100 then null 
  ...
  LEFT JOIN `physionet-data.mimiciii_clinical.labevents` le
    ...
    AND le.itemid IN
    (
      50810, -- HEMOGLOBIN | HEMATOLOGY | BLOOD | 752523
      51221
    )
...
SELECT r.subject_id, r.hadm_id, r.icustay_id, min(r.intime) as admission_time, min(r.charttime) as first_measurement
  , max(case when label = 'Hct' then valuenum else null end) as Hct_1st
FROM ranked r
